...
The **Console** is ProwlrBot's built-in web interface. After running `prowlr app`,
open `http://127.0.0.1:8088/` in your browser to enter the Console.

**In the Console, you can:**

- Chat with ProwlrBot in real time
- Enable/disable messaging channels
- View and manage all chat sessions
- Manage scheduled jobs
- Edit ProwlrBot's persona and behavior files
- Enable/disable skills to extend ProwlrBot's capabilities
- Manage MCP clients
...
button (↑). ProwlrBot replies in real time.
...
2. Fill in each section:
   ...
   - **Task Type & Content** — **Text** (fixed message) or **Agent** (ask
     ProwlrBot and forward reply), then the content
...
first (`pip install 'prowlr[llamacpp]'` or `pip install 'prowlr[mlx]'`).
...
- Install the Ollama SDK: `pip install 'prowlr[ollama]'` (or re-run the installer with `--extras ollama`)
...
- Models come from the Ollama daemon (not downloaded directly by ProwlrBot)
- Model list is auto-synced with Ollama
- Popular model examples: `mistral:7b`, `qwen3:8b`

> You can also manage Ollama models via CLI: `prowlr models ollama-pull`,
> `prowlr models ollama-list`, `prowlr models ollama-remove`. See
...
> **Note:** Cloud API key validity is your responsibility. ProwlrBot does not
...
> **Note:** Variable validity is your responsibility. ProwlrBot only stores and
> loads values.
>
...
| --------------------- | -------------------------------- | ---------------------------------------------------- |
| Chat                  | Chat → Chat                      | Talk with ProwlrBot, manage sessions                     |
